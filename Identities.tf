...
resource "azurerm_user_assigned_identity" "sb_data_receiver" {
  ...
  name                = format("id-sbdatareceiver-%s", azurerm_servicebus_subscription.subscription.name)
  resource_group_name = var.resource_group_name
  tags                = var.default_tags
}

resource "azurerm_role_assignment" "rbac_sb_data_receiver" {
  scope                = azurerm_servicebus_subscription.subscription.id
...
resource "azurerm_user_assigned_identity" "sb_data_sender" {
  location            = var.location
  name                = format("id-sbdatasender-%s", azurerm_servicebus_subscription.subscription.name)
  resource_group_name = var.resource_group_name
  tags                = var.default_tags
}
